...
void registrarPedidoBD(sqlite3 *db, Pedido *p, char *respuesta) {
    ...
    sprintf(sql_pedido,
            "INSERT INTO pedido (id_usuario, fecha) VALUES (%d, '%s');",
            p->id_usuario,
            p->fecha);
    ...
    while (actual != NULL) {
        ...
        sprintf(sql_linea,
                "INSERT INTO linea_pedido (id_pedido, id_producto, cantidad) "
                "VALUES (%I64d, %d, %d);",
                id_pedido,
                actual->id_producto,
                actual->cantidad);
        ...
        if (sqlite3_exec(db, sql_stock, 0, 0, 0) != SQLITE_OK) {
            sqlite3_exec(db, "ROLLBACK;", 0, 0, 0);
            strcpy(respuesta, "ERROR;Error actualizando stock");
            return;
        }

        if (sqlite3_changes(db) == 0) {
            sqlite3_exec(db, "ROLLBACK;", 0, 0, 0);
            strcpy(respuesta, "ERROR;Stock insuficiente o producto inexistente");
            return;
        }
...
void consultarPedidosBD(sqlite3 *db, int id_usuario, char *respuesta) {
    ...
    char *sql =
        "SELECT p.id, p.fecha, lp.id_producto, prod.nombre, lp.cantidad "
        "FROM pedido p "
